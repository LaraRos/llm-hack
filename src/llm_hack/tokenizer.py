...
class Tokenizer:
    ...
    def get_encoded_token(self, token: str) -> str:
        return self.str_to_int[token] if token in self.str_to_int else self.str_to_int[self.UNKNOWN]
    ...
    def decode(self, encoded_text: List[int]) -> str:
        text = " ".join([self.get_decoded_token(encoded_token) for encoded_token in encoded_text])
